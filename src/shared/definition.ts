...
export function makeField(type: string): Field {
  ...
  return { id: newId(), name: `${type}_${newId().slice(0, 6)}`, type, label: item?.label || type, required: type === 'recaptcha', placeholder: '', help: '', default: '', width: 12, options: ['First Choice', 'Second Choice', 'Third Choice'].map((label, i) => ({ label, value: `option_${i + 1}` })), children: [], ...(type === 'calculation' ? { formula: '0' } : {}), ...(type === 'grid' ? { rows: ['First row', 'Second row'] } : {}), ...(type === 'recaptcha' ? { extension: { theme: 'light', size: 'normal' } } : {}) };
}
